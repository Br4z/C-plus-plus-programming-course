	...
	if (!tree) tree = createNode(tree, value);
		...
		while (aux) {
			...
			if (value < aux -> value)
				...
				aux = aux -> right; // value >= aux -> value
		}

		if (value < parent -> value)
			parent -> left = createNode(parent, value);
		else
			parent -> right = createNode(parent, value); // value >= aux -> value
	}
} */
...
void searchToDelete(Node* &tree, int value) {
	if (!tree)
		...
		return searchToDelete(tree -> left, value);
	else
		return searchToDelete(tree -> right, value); // value > tree -> value
}
